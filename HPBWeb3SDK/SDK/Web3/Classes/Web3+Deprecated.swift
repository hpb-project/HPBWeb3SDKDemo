...
public struct EventParser {
    ...
    var web3: web3
    public init? (web3 web3Instance: web3, event: ABIElement, contract: ContractProtocol, filter: EventFilter? = nil, forAddress: HPBAddress? = nil) {
    ...
    public func parseBlock(_ block: Block) -> Result<[EventParsingResult], Web3Error> {
        ...
        if (self.contractAddress == nil) {
            ...
            for transaction in block.transactions {
                ...
                case .hash(let hash):
                    let response = self.web3.hpb.getTransactionDetails(hash)
                    switch response {
                    case .failure(let error):
                        return Result.failure(error)
    ...
    public func parseTransactionByHash(_ hash: Data) -> Result<[EventParsingResult], Web3Error> {
        let response = web3.hpb.getTransactionReceipt(hash)
        switch response {
        case .failure(let error):
            return Result.failure(error)
    ...
    public func parseTransaction(_ transaction: HPBTransaction) -> Result<[EventParsingResult], Web3Error> {
