...
extension web3.Hpb {
    func sendTransaction(_ transaction: HPBTransaction, options: Web3Options, password:String = "BANKEXFOUNDATION", callback: @escaping Callback, queue: OperationQueue = OperationQueue.main) {
        print(transaction)
    ...
    func sendTransaction(_ transaction: HPBTransaction, options: Web3Options, password:String = "BANKEXFOUNDATION") -> Result<[String: String], Web3Error> {
...
//        }
//        guard let request = HPBTransaction.createRequest(method: JSONRPCmethod.sendTransaction, transaction: transaction, onBlock: nil, options: mergedOptions) else
//        {
...
//        }
    ...
    func call(_ transaction: HPBTransaction, options: Web3Options, onBlock:String = "latest", callback: @escaping Callback, queue: OperationQueue = OperationQueue.main) {
        print(transaction)
    ...
    func call(_ transaction: HPBTransaction, options: Web3Options, onBlock:String = "latest") -> Result<[String: String], Web3Error> {
        var externalResult: Result<[String:String], Web3Error>!
        let semaphore = DispatchSemaphore(value: 0)
        let callback = { (res: Result<AnyObject, Web3Error>) -> () in
            switch res {
...
//        guard let request = HPBTransaction.createRequest(method: .call, transaction: transaction, onBlock: onBlock, options: mergedOptions) else
//        {
...
//        }
    ...
    func sendRawTransaction(_ transaction: HPBTransaction, callback: @escaping Callback, queue: OperationQueue = OperationQueue.main) {
        print(transaction)
    ...
    func sendRawTransaction(_ transaction: HPBTransaction) -> Result<[String: String], Web3Error> {
        var externalResult: Result<[String:String], Web3Error>!
        let semaphore = DispatchSemaphore(value: 0)
        let callback = { (res: Result<AnyObject, Web3Error>) -> () in
            switch res {
...
//        print(transaction)
//        guard let request = HPBTransaction.createRawTransaction(transaction: transaction) else {return Result.failure(Web3Error.transactionSerializationError)}
//        let response = self.provider.send(request: request)
//        let result = ResultUnwrapper.getResponse(response)
//        switch result {
//        case .failure(let error):
...
//        }
    ...
    public func getTransactionCount(address: HPBAddress, onBlock: String = "latest", callback: @escaping Callback, queue: OperationQueue = OperationQueue.main) {
        let operation = GetTransactionCountOperation.init(self.web3, queue: self.web3.queue, address: address, onBlock: onBlock)
        operation.next = OperationChainingType.callback(callback, queue)
        self.web3.queue.addOperation(operation)
    }
    
    public func getTransactionCount(address: HPBAddress, onBlock: String = "latest") -> Result<BigUInt, Web3Error> {
...
//        }
    ...
    public func getBalance(address: HPBAddress, onBlock: String = "latest") -> Result<BigUInt, Web3Error> {
        var externalResult: Result<BigUInt, Web3Error>!
        let semaphore = DispatchSemaphore(value: 0)
        let callback = { (res: Result<AnyObject, Web3Error>) -> () in
            switch res {
...
//        }
    ...
    public func getBalance(address: HPBAddress, onBlock: String = "latest", callback: @escaping Callback, queue: OperationQueue = OperationQueue.main) {
...
//        }
    ...
    public func estimateGas(_ transaction: HPBTransaction, options: Web3Options?, onBlock: String = "latest", callback: @escaping Callback, queue: OperationQueue = OperationQueue.main) {
    ...
    public func estimateGas(_ transaction: HPBTransaction, options: Web3Options?, onBlock: String = "latest") -> Result<BigUInt, Web3Error> {
...
//        guard let request = HPBTransaction.createRequest(method: JSONRPCmethod.estimateGas, transaction: transaction, onBlock: onBlock, options: mergedOptions) else {
...
//        }
    ...
    public func getAccounts() -> Result<[HPBAddress],Web3Error> {
        ...
        var externalResult: Result<[HPBAddress], Web3Error>!
        let semaphore = DispatchSemaphore(value: 0)
        let callback = { (res: Result<AnyObject, Web3Error>) -> () in
            switch res {
            case .success(let result):
                guard let unwrappedResult = result as? [HPBAddress] else {
                    externalResult = Result.failure(Web3Error.dataError)
                    break
                }
                externalResult = Result<[HPBAddress], Web3Error>(unwrappedResult)
            case .failure(let error):
                externalResult = Result.failure(error)
                break
            }
...
//                return Result.failure(Web3Error.dataError)
//                }
//            var toReturn = [HPBAddress]()
//            for addrString in resultArray {
//                guard let addr = HPBAddress(addrString) else {
//                    return Result.failure(Web3Error.dataError)
//                }
...
//        }
    ...
    public func sendETH(to: HPBAddress, amount: BigUInt, extraData: Data = Data(), options: Web3Options? = nil) -> TransactionIntermediate? {
    ...
    public func sendETH(to: HPBAddress, amount: String, units: Web3.Utils.Units = .hpb, extraData: Data = Data(), options: Web3Options? = nil) -> TransactionIntermediate? {
        guard let value = Web3.Utils.parseToBigUInt(amount, units: .hpb) else {return nil}
        return sendETH(to: to, amount: value, extraData: extraData, options: options)
    }
    
    public func sendERC20tokensWithKnownDecimals(tokenAddress: HPBAddress, from: HPBAddress, to: HPBAddress, amount: BigUInt, options: Web3Options? = nil) -> TransactionIntermediate? {
    ...
    public func sendERC20tokensWithNaturalUnits(tokenAddress: HPBAddress, from: HPBAddress, to: HPBAddress, amount: String, options: Web3Options? = nil) -> TransactionIntermediate? {
        let contract = self.web3.contract(Web3.Utils.erc20ABI, at: tokenAddress, abiVersion: 2)
        guard var mergedOptions = Web3Options.merge(self.options, with: options) else {return nil}
        mergedOptions.from = from
