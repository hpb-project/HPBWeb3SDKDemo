...
public struct TransactionDetails {
    ...
    public init? (_ json: [String: AnyObject]) {
        ...
        guard let transaction = HPBTransaction.fromJSON(json) else {return nil}
...
public struct TransactionReceipt {
    ...
    public var blockNumber: BigUInt
    public var transactionIndex: BigUInt
    public var contractAddress: HPBAddress?
    public var cumulativeGasUsed: BigUInt
    public var gasUsed: BigUInt
    public var logs: [EventLog]
    public var status: TXStatus
    public var logsBloom: HPBBloomFilter?
    ...
    public init(transactionHash: Data, blockHash: Data, blockNumber: BigUInt, transactionIndex: BigUInt, contractAddress: HPBAddress?, cumulativeGasUsed: BigUInt, gasUsed: BigUInt, logs: [EventLog], status: TXStatus, logsBloom: HPBBloomFilter?) {
    ...
    public init? (_ json: [String: AnyObject]) {
        ...
        if ca != nil {
            contractAddress = HPBAddress(ca!.addHexPrefix())
        }
        ...
        if lbl != nil {
            ...
            if logsData != nil && logsData!.count > 0 {
                logsBloom = HPBBloomFilter(logsData!)
            }
        }
    }
...
public struct EventLog {
    ...
    public init? (_ json: [String: AnyObject]) {
        ...
        guard let addr = HPBAddress(ad) else {return nil}
        address = addr
        data = Data.fromHex(d)!
...
public enum TransactionInBlock:Decodable {
    case hash(Data)
    case transaction(HPBTransaction)
    case null
    ...
    public init(from decoder: Decoder) throws {
        ...
        if let string = try? value.decode(String.self) {
            ...
            guard let t = HPBTransaction.fromJSON(dict) else {throw Web3Error.dataError}
            self = .transaction(t)
        } else {
            self = .null
        }
    ...
    public init?(_ data: AnyObject) {
        if let string = data as? String {
            ...
            guard let t = HPBTransaction.fromJSON(dict) else {return nil}
            self = .transaction(t)
        } else {
            return nil
        }
...
public struct Block:Decodable {
    ...
    public init(from decoder: Decoder) throws {
        ...
        var bloom:HPBBloomFilter?
        if logsBloomData != nil {
            bloom = HPBBloomFilter(logsBloomData!)
        }
        ...
        var miner:HPBAddress?
        if minerAddress != nil {
            guard let minr = HPBAddress(minerAddress!) else {throw Web3Error.dataError}
            miner = minr
        }
        self.miner = miner
