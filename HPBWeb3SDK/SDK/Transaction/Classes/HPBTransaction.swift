...
public struct HPBTransaction: CustomStringConvertible {
    ...
    public init(gasPrice: BigUInt, gasLimit: BigUInt, to: HPBAddress, value: BigUInt, data: Data) {
    ...
    public init(to: HPBAddress, data: Data, options: Web3Options) {
    ...
    public init (nonce: BigUInt, gasPrice: BigUInt, gasLimit: BigUInt, to: HPBAddress, value: BigUInt, data: Data, v: BigUInt, r: BigUInt, s: BigUInt) {
        self.nonce = nonce
        self.gasPrice = gasPrice
        self.gasLimit = gasLimit
        self.to = to
    ...
    public func encodeAsDictionary(from: HPBAddress) -> TransactionParameters? {
        if (!from.isValid) {
            return nil
        }
    ...
    static func fromJSON(_ json: [String: Any]) -> HPBTransaction? {
        ...
        if toString == "0x" || toString == "0x0" {
            to = HPBAddress.contractDeploymentAddress()
        } else {
            guard let hpbAddr = HPBAddress(toString) else {return nil}
            to = hpbAddr
        }
//        if (!to.isValid) {
//            return nil
//        }
        ...
        var transaction = HPBTransaction(to: to, data: data, options: options)
...
//        }
    ...
    static func fromRaw(_ raw: Data) -> HPBTransaction? {
        ...
        case 9?:
            ...
            case .data(let addressData):
                if addressData.count == 0 {
                    ...
                    guard let addr = HPBAddress(addressData) else {return nil}
                    to = addr
                } else {
                    return nil
                }
            ...
            let s = BigUInt(sData)
            return HPBTransaction.init(nonce: nonce, gasPrice: gasPrice, gasLimit: gasLimit, to: to, value: value, data: transactionData, v: v, r: r, s: s)
        case 6?:
            return nil
        default:
            return nil
    ...
    static func createRequest(method: JSONRPCmethod, transaction: HPBTransaction, onBlock: String? = nil, options: Web3Options?) -> JSONRPCrequest? {
    ...
    static func createRawTransaction(transaction: HPBTransaction) -> JSONRPCrequest? {
