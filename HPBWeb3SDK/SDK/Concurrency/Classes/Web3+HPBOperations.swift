...
final class GetTransactionCountOperation: Web3Operation {
    convenience init(_ web3Instance: web3, queue: OperationQueue? = nil, address: HPBAddress, onBlock: String = "latest") {
    ...
    override func main() {
        ...
        guard let _ = HPBAddress(address) else {return processError(Web3Error.inputError("Invalid input supplied"))}
...
final class EstimateGasOperation: Web3Operation {
    convenience init(_ web3Instance: web3, queue: OperationQueue? = nil, transaction: HPBTransaction, options: Web3Options?,  onBlock: String = "latest") {
    ...
    override func main() {
        ...
        guard let transaction = input[0] as? HPBTransaction else {return processError(Web3Error.inputError("Invalid input supplied"))}
        ...
        guard let request = HPBTransaction.createRequest(method: JSONRPCmethod.estimateGas, transaction: transaction, onBlock: onBlock, options: mergedOptions) else {return processError(Web3Error.inputError("Invalid input supplied"))}
        let dataOp = DataFetchOperation(self.web3, queue: self.expectedQueue)
        dataOp.inputData = request as AnyObject
        let parsingOp = BigUIntConversionOperation(self.web3, queue: self.expectedQueue)
        dataOp.next = OperationChainingType.operation(parsingOp)
...
final class CallOperation: Web3Operation {
    ...
    override func main() {
        ...
        guard let request = HPBTransaction.createRequest(method: JSONRPCmethod.call, transaction: transaction, onBlock: onBlock, options: mergedOptions) else {return processError(Web3Error.inputError("Invalid input supplied"))}
        let dataOp = DataFetchOperation(self.web3, queue: self.expectedQueue)
        dataOp.inputData = request as AnyObject
...
final class SendTransactionOperation: Web3Operation {
    convenience init(_ web3Instance: web3, queue: OperationQueue? = nil, transaction: HPBTransaction, options: Web3Options?, password:String = "BANKEXFOUNDATION") {
...
//        guard let request = HPBTransaction.createRawTransaction(transaction: transaction) else {return processError(Web3Error.transactionSerializationError)}
        let dataOp = DataFetchOperation(self.web3, queue: self.expectedQueue)
        dataOp.inputData = request as AnyObject
        let parsingOp = DataConversionOperation(self.web3, queue: self.expectedQueue)
        dataOp.next = OperationChainingType.operation(parsingOp)
...
final class SendRawTransactionOperation: Web3Operation {
    convenience init(_ web3Instance: web3, queue: OperationQueue? = nil, transaction: HPBTransaction) {
    ...
    override func main() {
        ...
        guard let completion = self.next else {return processError(Web3Error.inputError("Invalid input supplied"))}
        guard inputData != nil else {return processError(Web3Error.inputError("Invalid input supplied"))}
        guard let transaction = inputData! as? HPBTransaction else {return processError(Web3Error.inputError("Invalid input supplied"))}
        guard let request = HPBTransaction.createRawTransaction(transaction: transaction) else {return processError(Web3Error.transactionSerializationError)}
        let dataOp = DataFetchOperation(self.web3, queue: self.expectedQueue)
        dataOp.inputData = request as AnyObject
