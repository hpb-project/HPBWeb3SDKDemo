...
public class KeystoreManager: AbstractKeystore {
    ...
    public func UNSAFE_getPrivateKeyData(password: String, account: HPBAddress) throws -> Data {
    ...
    public func walletForAddress(_ address: HPBAddress) -> AbstractKeystore? {
        for keystore in _keystores {
            guard let key = keystore.addresses?.first else {continue}
    ...
    public init(_ keystores: [HPBKeystoreV3]) {
    ...
    private init?(_ path: String, scanForHDwallets: Bool = false, suffix: String? = nil) throws {
        ...
        if (suffix != nil) {
            for file in allFiles where file.hasSuffix(suffix!) {
                ...
                if (!scanForHDwallets) {
                    guard let keystore = HPBKeystoreV3(content) else {continue}
