...
extension web3.BrowserFunctions {
    ...
    public func getAccounts() -> [String]? {
        let result = self.web3.hpb.getAccounts()
        switch result {
        case .failure(_):
            return nil
    ...
    public func sign(_ personalMessage: Data, account: String, password: String = "BANKEXFOUNDATION") -> String? {
        do {
            ...
            guard let signature = try Web3Signer.signPersonalMessage(personalMessage, keystore: keystoreManager, account: HPBAddress(account)!, password: password) else {return nil}
    ...
    public func sendTransaction(_ transactionJSON: [String: Any], password: String = "BANKEXFOUNDATION") -> [String:Any]? {
        guard let transaction = HPBTransaction.fromJSON(transactionJSON) else {return nil}
    ...
    public func sendTransaction(_ transaction: HPBTransaction, options: Web3Options, password: String = "BANKEXFOUNDATION") -> [String:Any]? {
        let result = self.web3.hpb.sendTransaction(transaction, options: options, password: password)
        switch result {
        case .failure(_):
            return nil
    ...
    public func estimateGas(_ transaction: HPBTransaction, options: Web3Options) -> BigUInt? {
        let result = self.web3.hpb.estimateGas(transaction, options: options)
        switch result {
        case .failure(_):
            return nil
        case .success(let res):
    ...
    public func prepareTxForApproval(_ transactionJSON: [String: Any]) -> (transaction: HPBTransaction?, options: Web3Options?) {
        guard let transaction = HPBTransaction.fromJSON(transactionJSON) else {return (nil, nil)}
    ...
    public func prepareTxForApproval(_ trans: HPBTransaction, options  opts: Web3Options) -> (transaction: HPBTransaction?, options: Web3Options?) {
        var transaction = trans
        var options = opts
        guard let _ = options.from else {return (nil, nil)}
        let gasPriceResult = self.web3.hpb.getGasPrice()
    ...
    public func signTransaction(_ trans: HPBTransaction, options: Web3Options, password: String = "BANKEXFOUNDATION") -> String? {
        do {
            var transaction = trans
            ...
            let nonceResult = self.web3.hpb.getTransactionCount(address: from, onBlock: "pending")
