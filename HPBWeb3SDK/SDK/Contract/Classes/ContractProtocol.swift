...
public protocol ContractProtocol {
    var address: HPBAddress? {get set}
    var options: Web3Options? {get set}
    var allMethods: [String] {get}
    var allEvents: [String] {get}
    func deploy(bytecode:Data, parameters: [AnyObject], extraData: Data, options: Web3Options?) -> HPBTransaction?
    func method(_ method:String, parameters: [AnyObject], extraData: Data, options: Web3Options?) -> HPBTransaction?
    init?(_ abiString: String, at: HPBAddress?)
    ...
    func testBloomForEventPrecence(eventName: String, bloom: HPBBloomFilter) -> Bool?
...
extension HPBAddress: EventFilterable {
}
...
public struct EventFilter {
    ...
    public var addresses: [HPBAddress]?
