...
public struct ContractV2:ContractProtocol {
    ...
    public var address: HPBAddress? = nil
    ...
    public init?(_ abiString: String, at: HPBAddress? = nil) {
        do {
    ...
    public init(abi: [ABIv2.Element], at: HPBAddress) {
        _abi = abi
        address = at
    }
    
    public func deploy(bytecode:Data, parameters: [AnyObject] = [AnyObject](), extraData: Data = Data(), options: Web3Options?) -> HPBTransaction? {
        let to:HPBAddress = HPBAddress.contractDeploymentAddress()
        ...
        let transaction = HPBTransaction(gasPrice: gasPrice, gasLimit: gasLimit, to: to, value: value, data: fullData)
        return transaction
    }
    
    
    public func method(_ method:String = "fallback", parameters: [AnyObject] = [AnyObject](), extraData: Data = Data(), options: Web3Options?) -> HPBTransaction? {
        var to:HPBAddress
        let mergedOptions = Web3Options.merge(self.options, with: options)
        if (self.address != nil) {
            to = self.address!
        ...
        if (method == "fallback") {
            let transaction = HPBTransaction(gasPrice: gasPrice, gasLimit: gasLimit, to: to, value: value, data: extraData)
            return transaction
        }
        ...
        let transaction = HPBTransaction(gasPrice: gasPrice, gasLimit: gasLimit, to: to, value: value, data: encodedData)
        return transaction
    }
    ...
    public func testBloomForEventPrecence(eventName: String, bloom: HPBBloomFilter) -> Bool? {
