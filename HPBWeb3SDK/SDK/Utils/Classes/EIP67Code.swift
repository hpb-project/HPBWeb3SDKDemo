...
extension Web3 {
    ...
    public struct EIP67Code {
        ...
        public init (address : HPBAddress) {
            self.address = address
        }
        
        public init? (address : String) {
            guard let addr = HPBAddress(address) else {return nil}
            self.address = addr
        }
        
        public func toString() -> String {
            var urlComponents = URLComponents()
            let mainPart = "HPB:"+self.address.address.lowercased()
    ...
    public struct EIP67CodeParser {
        ...
        public static func parse(_ string: String) -> EIP67Code? {
            guard string.hasPrefix("HPB:") else {return nil}
            let striped = string.components(separatedBy: "HPB:")
            ...
            guard let address = HPBAddress(url.lastPathComponent) else {return nil}
